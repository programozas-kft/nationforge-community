    ...
    <div class="tl-item">
        ...
        <div class="tl-content">
            ...
            <ul>
                ...
                <li><strong>Nyilvános esemény-regisztráció (új funkció):</strong> Minden <em>published</em> státuszú eseményhez egyedi nyilvános regisztrációs oldal érhető el (<code>/e/{slug}</code>), bejelentkezés nélkül. A látogató megadja nevét, e-mail címét, telefonszámát, kísérők számát és megjegyzését. Kapacitáskorlát esetén vizuális töltöttségsáv jelenik meg, és ha az esemény betelt, a form helyett hibaüzenet látható. Sikeres regisztráció után visszaigazoló oldal fogadja a résztvevőt. Az admin Esemény részletoldalán megjelenik a regisztráltak listája (név, e-mail, telefon, kísérők, időpont), valamint egy „Publikus oldal" gomb, amely közvetlenül a nyilvános oldalra navigál.</li>
            </ul>
        </div>
    </div>
    ...
    <div class="tl-item">
        ...
        <div class="tl-content">
            ...
            <ul>
                ...
                <li><strong>Főoldal görgetés javítása:</strong> A layout főoszlopa explicit <code>height: calc(100vh - 38px)</code> magasságot kapott, a <code>&lt;main&gt;</code> elem pedig <code>flex:1; min-height:0</code> kombinációval tölti ki a maradék területet — így a hosszabb tartalmak helyesen görgethetők.</li>
                <li><strong>Panel padding egységesítés:</strong> Az Esemény részletoldal Részletek panelén és a főoldal Közelgő események listáján a Tailwind <code>px-5</code> osztályok helyett garantáltan érvényesülő inline <code>padding: 20px</code> stílusok kerültek be.</li>
                <li><strong>Linkgyűjtemény modul (új oldal):</strong> Új <code>/admin/links</code> oldal, amely a mentett hivatkozásokat kategóriánként csoportosítva, kártyás elrendezésben jeleníti meg. Minden kártya tartalmaz színes ikont, leírást és domain-chip feliratot; kattintásra új lapon nyílik meg.</li>
                <li><strong>Linkgyűjtemény konfigurálása a Beállításokban:</strong> A Beállítások oldal alján új szekció teszi lehetővé a linkek kezelését (hozzáadás, szerkesztés, törlés) — modal alapú felülettel, cím, URL, kategória, szín, leírás, sorrend és aktív/inaktív mezőkkel.</li>
                <li><strong>Gyorslinkek sáv — valódi URL-ek:</strong> A felső kék sávban lévő gyorslinkek megkapták a tényleges hivatkozásaikat: <em>YouTube</em>, <em>Google Drive</em>, <em>Instagram</em>, <em>Hírek</em> (Google News), <em>Infografikonok</em> (Google képkereső). Minden link új lapon nyílik meg. A „Linkgyűjtemény" gomb az új <code>/admin/links</code> oldalra navigál.</li>
                <li><strong>Beállítások oldal teljes szélességű elrendezése:</strong> A korábban középre igazított, korlátozott szélességű Beállítások felület mostantól a teljes rendelkezésre álló területet kitölti. Az Általános és Email szekciók kétoszlopos rácsban helyezkednek el egymás mellett, az info kártyák (PHP, Laravel, Környezet) a jobb oszlop alján sorakoznak.</li>
            </ul>
        </div>
    </div>
